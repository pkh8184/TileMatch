...
namespace TrumpTile.GameMain.Data
{
    public static class DataResetor
    {
        ...
        [MenuItem("Tools/Reset Champions Data")]
        public static void ResetChampionsData()
        {
            if(EditorUtility.DisplayDialog(
            "챔피언스 관련 데이터 초기화", 
            "챔피언스 관련 PlayerPrefs 데이터를 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.", 
            "삭제", 
            "취소"))
            {
                PlayerPrefs.DeleteKey("LeaderboardData");
                PlayerPrefs.DeleteKey("Leaderboard_LastRefresh");
                PlayerPrefs.Save();
                Debug.Log("[DataReset] 챔피언스 관련 PlayerPrefs 데이터가 삭제되었습니다.");
            }
        }
